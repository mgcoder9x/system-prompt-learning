...
- [x] 2. Nền schema: model + schemas/ + drift-guard cho `blueprint` và `area_refs`
...
- [x] 2.2 Thêm pydantic model `Blueprint`, `MandatoryArea` + field `CurriculumPoint.area_refs`
...
- [x] 3. Blueprint_Validator — mã lỗi cấu trúc (RED-first từng mã), sau khi CR-0011 approved
- [x] 3.1 `E-BP-DUP-ID` + `E-BP-ORDER` + `E-BP-EMPTY-TITLE`
...
- [x] 3.2 `E-BP-REF-BROKEN` (source_refs blueprint trỏ file reference/ tồn tại)
  - Test đỏ; hiện thực resolve tương đối `topic_dir` (song song E-CURR-REF-BROKEN, abspath do E-PORT-ABSPATH).
  - _Requirements: 2.2, 6.1_
- [x] 3.3 `E-BP-AREA-REF-BROKEN` (curriculum point.area_refs trỏ area tồn tại)
...
- [x] 4. Blueprint_Validator — kiểm phủ (approved-gated), RED-first
- [x] 4.1 `E-BP-AREA-UNCOVERED` (area mandatory chưa phủ khi blueprint approved)
...
- [x] 4.2 `E-BP-POINT-OUTSIDE` (point không ánh xạ area nào khi blueprint approved)
...
- [x] 5. Lệnh blueprint (build/edit/approve/amend) — sau CR-0013 approved
- [x] 5.1 `cmd_blueprint` build (draft) transaction-FULL
...
- [x] 5.2 `cmd_blueprint --edit` (chỉ draft) + `--approve` (draft→approved gate validator PASS)
...
- [x] 5.3 `cmd_blueprint --amend --confirm` (sửa approved có kiểm soát)
...
- [x] 6. Curriculum mang `area_refs` + wiring phủ — sau CR-0012/0013 approved
...
- [x] 7. Nghiệm thu E2E tất định + bảo toàn bất biến
- [x] 7.1 E2E chuỗi thật trên vault tmp
...
- [x] 7.2 Backward-compat + bất biến nền
...
- [x] 8. Áp change-request + tài liệu + spec + DEC (sau khi code XANH)
