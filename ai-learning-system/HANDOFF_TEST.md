...
> phần bắt buộc và bạn phải làm **ĐẾN HẾT** (A1 + A2 + A3) rồi mới coi là xong phần máy. Phần B cần **người
...
**A3 — Vòng khung bắt buộc v2.8 (blueprint + retrofit `area_refs` — làm SOLO, KHÔNG cần người):**
Sau A2, topic `<id>` đã có curriculum teachable (các điểm CHƯA gắn khung). A3 dựng khung bắt buộc rồi
**retrofit** để phủ đủ — chứng minh luồng "dựng-giáo-trình-TRƯỚC rồi áp-khung-SAU" (giải ngõ cụt NOTE-039).
11. Dựng khung (draft): `... session.py blueprint --system . --vault ..\learning_vault --topic <id> --areas "[{\"title\": \"Mảng nền tảng\"}, {\"title\": \"Mảng nâng cao\"}]"` → `committed: true` (tạo `ma-001`, `ma-002`, `status=draft`).
12. Đọc `topics/<id>/curriculum.md` lấy các `cp-id`. Retrofit MỖI điểm map ≥1 mảng (sao cho MỌI điểm có `area_refs` VÀ cả `ma-001`+`ma-002` đều được phủ), ví dụ:
    `... session.py curriculum --system . --vault ..\learning_vault --topic <id> --set-area-refs cp-001 --area-refs "[\"ma-001\"]"` (lặp cho các điểm còn lại: `cp-002`→`["ma-002"]`, `cp-003`→`["ma-001"]`, ...).
13. Duyệt khung: `... session.py blueprint --system . --vault ..\learning_vault --topic <id> --approve` → `committed: true`. (Nếu CHƯA phủ đủ → `committed: false` kèm `E-BP-AREA-UNCOVERED`/`E-BP-POINT-OUTSIDE` — ĐÚNG thiết kế cổng phủ; retrofit tiếp cho đủ rồi approve lại.)
14. `... validator\validate.py --system . --vault ..\learning_vault --level full --scope full --json` → phải `pass: true` (toàn vẹn sau cả vòng khung bắt buộc).

**Tiêu chí đạt Phần A:** mọi lệnh ghi để lại vết trong transaction log; `validate.py --scope full` trả
`pass: true` ở **bước 10 VÀ bước 14**; blueprint chỉ `approve` được SAU khi retrofit phủ đủ (chứng minh cổng
phủ teachable-gated); KHÔNG crash traceback (lỗi phải là mã `E-*` sạch nếu có).
...
- A3 (blueprint draft → set-area-refs retrofit → approve) — lệnh + kết quả từng bước:
- validate.py --scope full ở bước 10 + bước 14: <pass?>
...
| R4c | Đọc `topics/<id>/blueprint.md` + `curriculum.md` (Phần A3) | blueprint `status: approved`; mỗi Mandatory_Area bắt buộc được phủ bởi ≥1 điểm (`area_refs`); mọi điểm map ≥1 mảng; approve chỉ thành công SAU retrofit; `validate --scope full` PASS |
...
**Kết luận test:** ĐẠT nếu R1–R4c đạt (bảo đảm Class A portable + vòng giáo trình v2.7 + vòng khung bắt buộc
v2.8 còn nguyên) **và** R7 trung thực. R5/R6 tốt là điểm cộng Class D/quy trình (R5 bỏ được nếu test SOLO
không có người học). R3/R4b/R4c fail = **hệ mất toàn vẹn khi sang AI khác** → điều tra gốc.
...
commands: ["/learn", "/validate", "/status", "/schedule", "/collect", "/curriculum",
           "/blueprint", "/next-lesson", "/grade", "/review", "/done"]
```
