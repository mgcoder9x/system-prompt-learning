...
def _write_curriculum(td: Path, area_refs_by_point: list[list[str]]) -> None:
    td.mkdir(parents=True, exist_ok=True)
    pts = ""
...
def test_draft_uncovered_no_error(tmp_path):
    """P9 backward-compat: draft blueprint KHÔNG ép phủ."""
    v = tmp_path / "vault"; td = v / "topics" / "docker"
    _write_blueprint(td, _area("ma-001", 1) + _area("ma-002", 2), status="draft")
    _write_curriculum(td, [["ma-001"]])  # ma-002 chưa phủ nhưng draft → không lỗi
...
def test_approved_no_curriculum_uncovered(tmp_path):
    """Approved blueprint nhưng CHƯA có curriculum → mọi area mandatory chưa phủ."""
    v = tmp_path / "vault"; td = v / "topics" / "docker"
